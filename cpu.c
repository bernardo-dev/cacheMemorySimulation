...
void executeInstruction(Machine *machine, int PC) {
  ...
  case 0: // Taking information to RAM
    ...
    line = MMUSearchOnMemorys(add2, machine,
                              &hit); /* Searching block on memories */
#ifdef PRINT_LOG
    printf("BLOCK[%s.%d.%d](%4d)\n", convertToString(hit), add2.block,
           add2.word, line->block.words[add2.word]);
#endif
    ...
    break;
  case 1: // Sum
    line = MMUSearchOnMemorys(add1, machine,
                              &hit); /* Searching block on memories */
    word1 = line->block.words[add1.word];
#ifdef PRINT_LOG
    printf("  > SUM BLOCK[%s.%d.%d](%4d)", convertToString(hit), add1.block,
           add1.word, line->block.words[add1.word]);
#endif
...
#ifdef PRINT_LOG
    printf(" + BLOCK[%s.%d.%d](%4d)", convertToString(hit), add2.block,
           add2.word, line->block.words[add2.word]);
#endif

    line = MMUSearchOnMemorys(add3, machine,
                              &hit); /* Searching block on memories */
#ifdef PRINT_LOG
    printf(" > BLOCK[%s.%d.%d](%4d|", convertToString(hit), add3.block,
           add3.word, line->block.words[add3.word]);
#endif
  ...
  case 2: // Subtract
    ...
    word1 = line->block.words[add1.word];
#ifdef PRINT_LOG
    printf("  > SUB BLOCK[%s.%d.%d](%4d)", convertToString(hit), add1.block,
           add1.word, line->block.words[add1.word]);
#endif
  ...
  case 3: // Moving information in RAM
    ...
    word1 = line->block.words[add1.word];
#ifdef PRINT_LOG
    printf("  > MOV BLOCK[%s.%d.%d](%4d) > ", convertToString(hit), add1.block,
           add1.word, line->block.words[add1.word]);
#endif
    ...
    line = MMUSearchOnMemorys(add2, machine,
                              &hit); /* Searching block on memories */
#ifdef PRINT_LOG
    printf("BLOCK[%s.%d.%d](%4d|", convertToString(hit), add2.block, add2.word,
...
void run(Machine *machine) {
  ...
  while (machine->instructions[PC].opcode != -1) {
    ...
    printf("\tL1:(%6d, %6d) | L2:(%6d, %6d) | L3:(%6d, %6d) | RAM:(%6d) | "
           "COST: %d\n",
           machine->hitL1, machine->missL1, machine->hitL2, machine->missL2,
           machine->hitL3, machine->missL3, machine->hitRAM,
           machine->totalCost);
  }
}
...
void printcolored(int n, bool updated) {
  if (updated)
    printf("\x1b[1;37;42m[%3d]\x1b[0m", n); // Green
  else
    printf("\x1b[1;37;41m[%3d]\x1b[0m", n); // Red
}

void printMemories(Machine *machine) {
  printf("\x1b[0;30;47m   ");
  printc("RAM", WORDS_SIZE * 5 + 3);
  printc("Cache L3", WORDS_SIZE * 6 + 9);
  printf("%c", ' ');
  printc("Cache L2", WORDS_SIZE * 6 + 9);

  printf("%c", ' ');
  printc("Cache L1  ", WORDS_SIZE * 6.3 + 9);
  printf("\x1b[0m\n");
  for (int i = 0; i < machine->ram.size; i++) {
    printf("\x1b[0;30;47m%3d|\x1b[0m", i);
    for (int j = 0; j < WORDS_SIZE; j++) {
      printf(" %3d |", machine->ram.blocks[i].words[j]);
    }
    if (i < machine->l3.size) {
      printf("|");
      printcolored(machine->l3.lines[i].tag, machine->l3.lines[i].updated);
      for (int k = 0; k < WORDS_SIZE; k++) {
        printf(" %3d |", machine->l3.lines[i].block.words[k]);
      }
      printf(" %3d |", machine->l3.lines[i].queuePlace);
      if (i < machine->l2.size) {
        printf("|");
        printcolored(machine->l2.lines[i].tag, machine->l2.lines[i].updated);
        for (int j = 0; j < WORDS_SIZE; j++) {
          printf(" %3d |", machine->l2.lines[i].block.words[j]);
        }
        printf(" %3d |", machine->l2.lines[i].queuePlace);
        if (i < machine->l1.size) {
          printf("|");
          printcolored(machine->l1.lines[i].tag, machine->l1.lines[i].updated);
          for (int j = 0; j < WORDS_SIZE; j++) {
            printf(" %3d |", machine->l1.lines[i].block.words[j]);
          }
          printf(" %3d |", machine->l1.lines[i].queuePlace);
        }
      }
    }
    printf("\n");
